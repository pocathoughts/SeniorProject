...
/*
function GetAllClubsTrimmedName ($link, $data, &$returnData){
  GetAllClubsValidate($data, $returnData);

  $query = "SELECT CONCAT(club_team.club_name) AS display_string FROM club_operating_year, club_team, operating_year WHERE club_team.club_id = club_operating_year.club_id and operating_year.year_id = club_operating_year.year_id ORDER BY display_string";
  $results = queryMultiple($link, $query, "All Clubs Query", 5000);
...
*/
function GetClubNames ($link, $data, &$returnData){
  GetAllClubsValidate($data, $returnData);
